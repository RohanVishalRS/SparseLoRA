...
class RobertaSelfAttentionLora(RobertaSelfAttention):
    def __init__(self, config, rank=8, position_embedding_type=None):
    ...
    def forward(
        ...
        mixed_query_layer = self.query_lora(hidden_states)
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            value_layer = self.transpose_for_scores(self.value_lora(encoder_hidden_states))
            attention_mask = encoder_attention_mask
        elif past_key_value is not None:
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            value_layer = self.transpose_for_scores(self.value_lora(hidden_states))
